...
def main(page: ft.Page):
    page.title = "Shikongo Linus TK - SiteSpy Portfolio"
    ...
    hero = ft.Container(
        ...
        content=ft.Column([
            ...
            ft.Text("Shikongo Linus TK", size=34, weight=ft.FontWeight.W_900, color=TEXT),
            ft.Text("GitHub: Linus45-blid | Email: linusshikongo45@gmail.com", color=MUTED, selectable=True),
            ft.Text("Deployment documentation and EAS readiness.", size=16, color=TEXT),
            ft.Text("GitHub profile: https://github.com/Linus45-blid", color=GOLD, selectable=True),
        ], spacing=13),
    )
        ...
        ft.Container(
            ...
            content=ft.ResponsiveRow([
                ...
                ft.Container(tile('Contribution Area', [
                    ft.Text("Deployment documentation and EAS readiness.", color=TEXT),
                ]), col={'xs': 12, 'md': 6}),
                ft.Container(tile('Files / Validation Area', [
                    ft.Text('- eas.json', color=MUTED, size=13),
                    ft.Text('- docs/Final_Submission_Checklist.md', color=MUTED, size=13),
                    ft.Text('- README.md', color=MUTED, size=13),
                ], ACCENT), col={'xs': 12, 'md': 6}),
                ft.Container(tile('Local Git Setup', [
                    ft.Text("git config user.name \"Shikongo Linus TK\"\ngit config user.email \"linusshikongo45@gmail.com\"\ngit config --get user.name\ngit config --get user.email", color=TEXT, selectable=True),
                ]), col={'xs': 12, 'md': 6}),
                ft.Container(tile('Clone, Commit, Push', [
                    ft.Text("git clone \"Replace with your own fork URL after creating the fork.\"\ncd \"Replace with your cloned repository folder\"\ngit status\ngit checkout -b portfolio/shikongo-ltk-sitespy-portfolio\ngit add README.md index.html main.py\ngit commit -m \"Update Shikongo Linus TK SiteSpy portfolio\"\ngit push -u origin portfolio/shikongo-ltk-sitespy-portfolio", color=TEXT, selectable=True),
